...
void ListCollection::printFileHeader(std::ofstream &file) {
    file << "Vardas Pavarde ND1 ND2 ND3 ND4 ND5 Egzaminas" << '\n';
}

void ListCollection::printStudentToFile(std::ofstream &file, const Student &student) {
...
void ListCollection::writeStudentsByTypeToFile(string badStudentsFilename, string goodStudentsFilename) {
...
void ListCollection::generateRandomFile(string filename, int numOfStudents) {
    Student student;
...
void ListCollection::loadFromFile(string filename, int numHomeworkResults) {
...
Student ListCollection::getRandomStudent(int numOfHomework, int id) {
    Student student;
...
Student ListCollection::getStudentFromInput() {
    Student student;
...
void ListCollection::loadFromConsole(bool useRandom) {
    if (useRandom) {
...
void ListCollection::printResults() {
    Table table(3);
...
void ListCollection::sortByName() {
    students.sort();
}

void ListCollection::calculateMedian() {
    for (auto &student : students) {
        student.calculateMedian();
    }
}

void ListCollection::calculateAverage() {
    for (auto &student : students) {
        student.calculateAverage();
    }
}

void ListCollection::calculateFinal() {
...
string ListCollection::getFinalResultLabel() {
    const string modeLabel = (finalResultMode == 'v' ? "Vid." : "Med.");
    return "Galutinis (" + modeLabel + ")";
}

void ListCollection::setTypeByFinalResult() {
    for (auto &student : students) {
